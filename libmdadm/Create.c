	   ...
	   struct context *c, unsigned long long data_offset)
{
	...
	default:
		...
		return CREATE_RAID_LEVEL_UNKNOWN;
	}
	if (s->size == MAX_SIZE)
	...
	if (st && ! st->ss->validate_geometry(st, s->level, s->layout, s->raiddisks,
					      ...
					      data_offset, NULL,
					      &newsize, c->verbose>=0))
		return CREATE_GEOMETRY_INVALID;
	...
	for (dv=devlist; dv && !have_container; dv=dv->next, dnum++) {
		...
		if (dfd < 0) {
			...
			//exit(2);
			return CREATE_RAID_DEV_CANNOT_OPEN;
		}
		if (fstat(dfd, &stb) != 0 ||
			...
			pr_err("%s is not a block device\n",
				dname);
			//exit(2);
			return CREATE_NOT_BLOCK_DEVICE;
		}
		close(dfd);
		...
		if (st == NULL) {
			...
			if (!st) {
				...
				pr_err("device %s not suitable "
					...
					dname);
				//exit(2);
				return CREATE_UNSUITABLE_STYLE;
			}
			if (st->ss != &super0 ||
			    st->minor_version != 90)
				did_default = 1;
		...
		if (s->size && freesize < s->size) {
			...
			fail = CREATE_FREE_SPACE_SMALL_THAN_GIVEN;
			continue;
		}
		...
		if (c->runstop != 1 || c->verbose >= 0) {
			...
			if (fd <0 ) {
				pr_err("Cannot open %s: %s\n",
					dname, strerror(errno));
				fail=CREATE_RAID_DEV_CANNOT_OPEN;
				continue;
			}
	...
	if (s->size == 0) {
		if (mindisc == NULL && !have_container) {
			...
			return CREATE_WITHOUT_GIVEN_SIZE_DRIVES;
		}
	...
	if (s->level <= 0 && first_missing < subdevs * 2) {
		...
		return CREATE_MISSING_DEVS_NOT_SUPPORT;
	}
	...
	if (vers < 9000) {
		...
		fail = CREATE_REQUIRE_NEWER_VERSION;
		goto abort_locked;
	} else {
		mdu_array_info_t inf;
		memset(&inf, 0, sizeof(inf));
...
/*		printf("bms=%llu as=%d rd=%d nc=%d\n", bitmapsize, s->size, s->raiddisks, ncopies);*/
	...
	if (s->bitmap_file && strcmp(s->bitmap_file, "internal")==0) {
		if ((vers%100) < 2) {
			...
			fail = CREATE_BITMAP_NOT_SUPPORT;
			goto abort_locked;
		}
		...
		if (!st->ss->add_internal_bitmap(st, &s->bitmap_chunk,
			...
			fail = CREATE_BITMAP_CHUNK_NOT_SUPPORT;
			goto abort_locked;
		}
		s->bitmap_file = NULL;
	}
	...
	if (st->ss->external && st->container_devnm[0]) {
		...
		if (container_fd < 0) {
			...
			fail = CREATE_OPEN_CONTAINER_FAIL;
			goto abort_locked;
		}
	...
	if (rv) {
		...
		fail = CREATE_SET_ARRAY_INFO_FAIL;
		goto abort_locked;
	}
	...
	if (s->bitmap_file) {
		...
		if (bitmap_fd < 0) {
			pr_err("weird: %s cannot be openned\n",
				s->bitmap_file);
			fail = CREATE_OPEN_BITMAP_FILE_FAIL;
			goto abort_locked;
		}
		if (ioctl(mdfd, SET_BITMAP_FILE, bitmap_fd) < 0) {
			pr_err("Cannot set bitmap file for %s: %s\n",
				mddev, strerror(errno));
			fail = CREATE_SET_BITMAP_FILE_FAIL;
			goto abort_locked;
		}
	}
	...
	for (pass=1; pass <=2 ; pass++) {
		...
		if (pass == 1) {
			...
			if (st->ss->write_init_super(st)) {
				st->ss->free_super(st);
				fail = CREATE_FREE_SUPER_TYPE_OBJ_FAIL;
				goto abort_locked;
			}
		...
		if (st->ss->external) {
			...
			if (err) {
				pr_err("failed to"
				       " activate array.\n");
				ioctl(mdfd, STOP_ARRAY, NULL);
				fail = CREATE_RAID_ACTIVATION_FAIL;
				goto abort;
			}
		} else if (c->readonly &&
